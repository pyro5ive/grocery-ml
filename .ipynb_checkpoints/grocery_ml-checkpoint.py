...
class GroceryML:
    ...
    ###########################################################################################
    def build_combined_training_df(self, use_neg_samples):

        print(f"build_combined_df(use_neg_samples = {use_neg_samples})") 
        winndixie_df = self.build_winn_dixie_df(r"data\training\winndixie\txt");
        winndixie_add_txt_rpts_df = self.build_winn_dixie_additional_text_rcpts_df(r"data\training\winndixie\additionalTxtRcpts")
        winndixie_df = pd.concat([winndixie_df, winndixie_add_txt_rpts_df], ignore_index=True)
        #       
        wallmart_df = WallmartRecptParser.build_wall_mart_df("data\training\walmart");
        ...
        df_weather = WeatherFeatures.BuildWeather(r"data\training\weather").reset_index()
    ...
    def build_combined_df(self, use_neg_samples):
        ...
        winndixie_add_txt_rpts_df = self.build_winn_dixie_additional_text_rcpts_df(r"data\production\winndixie\txt")
        winndixie_df = pd.concat([winndixie_df, winndixie_add_txt_rpts_df], ignore_index=True)
        #       
    ...
    def run_experiment(self,  modelBuildParams, modelTrainParams, baseDir):

        df = self.combined_training_df;
        print(f"run_experiment()  baseDir: {baseDir}  ");
        print(f"run_experiment()  when: {datetime.now()} params: {modelTrainParams}  ");
                
        norm_params = self.fit_normalization_params(df)
        normalized_df = self.normalize_features(df, norm_params)
